     ...
     .  t2,ratio,bx(2),mintime,maxtime,ntrans(nplanetmax),flux2(nmax),
...
c        endif
 ...
 13     continue
        ...
        maxtime=mintime+period*ntrans(ii)
        k=0
        kk=0
        kk2=0
...
c                write(0,*) px(k),py(k)
            ...
            if((phase(j).lt.phd1).or.(phase(j).gt.phd2))then
                ...
                if(time(j).lt.maxtime) kk2=kk2+1
            endif
c            write(0,*) kk,kk2
c            read(5,*)
 14     continue
...
c        write(6,*) rbb(1),rbb(2),rbb(3),rbb(4)
 ...
 500    format(A4,F3.1)
        call pgptxt(rbb(2)-0.20*(rbb(2)-rbb(1)),rbb(3)
     .      +0.10*(rbb(4)-rbb(3)),0.0,0.5,label)
...
c        call pgline(2,px,py)
     ...
     .          0.5,"Scaled Transit Duration")
        ...
        call pgptxt(rbb(1)-0.25*(rbb(2)-rbb(1)),(rbb(3)+rbb(4))/2,
     .      90.0,0.5,"Flux (ppm)")
        ...
        call pgwindow(real(-width),real(width),1.0e6*(rbb(3)-1.0),
     .  1.0e6*(rbb(4)-1.0))        

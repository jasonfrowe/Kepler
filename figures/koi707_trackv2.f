     ...
     .  M(5),logg(nmax),G,age(nmax),x,y,rbb(4),oldage,newage,age1,Teff1,
     .  logg1,Teff2,logg2,age2,page,dage,pTeff,plogg
      character*80 filename(5),dumc,text,tfile
...
c 14   continue
 ...
 15   continue
      ...
      nunit=10
      filename(1)="m11.track2"
      filename(2)="m12.track2"
      filename(3)="m13.track2"
      filename(4)="m14.track2"
      filename(5)="m15.track2"
      M(1)=1.1
      M(2)=1.2
      M(3)=1.3
      M(4)=1.4
...
c            write(0,*) Teff(i),logg(i),M(j),trad
 ...
 500        format(F4.1)
            ...
            if((x.lt.rbb(1)).and.(x.gt.rbb(2)+50).and.
     .        (y.lt.rbb(3)).and.(y.gt.rbb(4)))then
                if((n1.eq.0).or.(newage.ne.oldage))then

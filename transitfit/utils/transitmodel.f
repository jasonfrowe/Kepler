...
C     parameters for stellar rotation
      double precision vrot,vrotf,bmin
C     Read in physical constants (Pi,G,Msun,..etc.)
      include "physcons.f"
      
      vrot=70.0d0*1000.0d0 !70 km/s rotation
...
C     calculated semi-major axis
      ...
      bmin=asemi/R1*cos(sol(6)*pi/180.0d0)
...
c                tflux=tides(M1,M2,R1,asemi,incl,phi(j),eccn,Pi)
                tflux=tflux+
     .              tides(M1,M2,R1,asemi,incl,Tanom(j),eccn,Pi,fT,phi0)
...
c                write(0,*) "bmin: ",bmin
c                read(5,*)
cNOOOOO                do 16 j=1,nintg
c                    vrotf=vrotf-1.0d1*(darea*norm-zarea-tflux-norm)*
c     .                  (b0(j)-bmin)*x2(j)/abs(x2(j))*vrot/Cs*fDB
cc                   vrotf=vrotf+(b0(j)-bmin)*x2(j)/abs(x2(j))*vrot/Cs*fDB
c 16             continue
c                vrotf=vrotf/dnintg
c                write(0,*) time(i)+54900.0d0-0.5d0,vrotf
c                read(5,*)

...
C     Physical parameters of the model
     ...
     .  incl,Per,phi,eccn,w,ted,inclmin
C     Parameters that work towards MandelAgol
     ...
     .  mulimb0(nintg),mulimbf(nintg,5),dist(nintg),et(2),etest
C     Albedo routines
...
c      et(1)=w+Pi-2.0*Pi*floor( (w+Pi)/(2.0*Pi) )
      et(1)=trueanomaly(-eccn,w)
      call invkepler(et(1),et(2),eccn)
c      write(6,*) "etest",et(1),et(2)
c      etest=et(1)-et(2)
      phi=2.0*pi-2.0*pi*(sol(7)/sol(5)-int(sol(7)/sol(5)))-et(2)
      if(phi.lt.0.0)phi=phi+tpi
c      phi=phi-2.0*Pi*floor(Phi/(2.0*Pi))
c      phi=dble(sol(7))
...
C     RV initialization
     ...
     .  (Psec*(1.0d0-eccn*eccn)**(3.0d0/2.0d0)*(M1+M2)*(M1+M2))
      K=K**(1.0d0/3.0d0)
      voff=sol(17) !velocity offset - zero for now
...
C           Solve Kepler to get distance
            Manom=tpi*t(j)/Per+phi !mean anomaly
            if(Manom.ge.tpi) Manom=Manom-tpi*floor(Manom/tpi)
            Eanom=Manom
            call kepler(Manom,Eanom,eccn)
            Tanom(j)=trueanomaly(eccn,Eanom)
            arad(j)=distance(asemi,eccn,Tanom(j))
            x2(j)=arad(j)*Sin(Tanom(j)+w)
            y2(j)=arad(j)*Sin(incl)*Cos(Tanom(j)+w)

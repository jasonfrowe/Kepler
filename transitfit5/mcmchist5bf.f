...
C     Now we read in MCMC results, cycling through as needed.
      ...
      call getbestfit(nunit,nfitm,nplanet,bsol)
      rewind(nunit)
      do i=1,nfitm
         bsol(i)=sol(i)
      enddo
...
c                    call rqsort(npt,dd,nd) !changed npt to k
c                    jj=npt/2
c                    if(jj.le.0) jj=1
                    med=bsol(ii)!dd(nd(jj))
                    do 15 jj=1,npt
...
!                    call writetable(ave,std,errs,name)
                    ...
                    call histogram(npt,rp,dd,work,nd,nbin,nbinmax,
     .               bdatax,bdatay,title,ave,std,errs,bsol(ii))
                    if(j.eq.1)then
                     call writetable(real(sol(ii)),std,errs,name)
                    else
                     call writetable(ave,std,errs,name)
...
!                  output(4)=ave
 ...
 12     continue
        ...
        call getadrs(nunit,i,npt,dd,np,rstar,mstar,rhostar,seed,nfitm,
     .   bsol,bs)
        title="a/R\d\(2281)\u"
        call histogram(npt,rp,dd,work,nd,nbin,nbinmax,bdatax,
     .      bdatay,title,ave,std,errs,bs)
...
c        rewind(nunit)
        ...
        call gettdep(nunit,i,npt,dd,bsol,bs)
...
c        rewind(nunit)
        ...
        call gettdur2(nunit,i,nmax,npt,dd,np,rstar,mstar,seed,nfitm,
     .   bsol,bs)
        title="T\dd\u (h)"
        call histogram(npt,rp,dd,work,nd,nbin,nbinmax,bdatax,
     .      bdatay,title,ave,std,errs,bs)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine getbestfit(nunit,nfitm,nplanet,bsol)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nunit,nfitm,nplanet,i,j,flag,ng
      double precision bsol(nfitm),tsol(nfitm),chi,bchi,dumi,dumr
 ...
 10   read(nunit,*,end=11) chi,flag,ng,(tsol(i),i=1,nplanet*10+8)
         if(chi.lt.bchi)then
            bchi=chi
            do 12 j=1,nplanet*10+8
               bsol(j)=tsol(j)
 12         continue
         endif
      goto 10
 11   continue
c      write(0,*) "bfr: ",bsol(1)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine gettdep(nunit,nplanet,npt,tdep,bsol,bs)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nunit,nplanet,npt,j,col,i,dtype(1),nfitm,nplanetmax,nfit,
     .   nmax,ii
      parameter(nplanetmax=10,nmax=2000000,nfitm=112)
...
C       check that tdep is a valid number.
        if((tdep(i).gt.-1.0d30).and.(tdep(i).lt.1.0d30)) i=i+1
        goto 10
 11   continue
 ...
 13   continue
      ...
      call transitmodel(nfit,1,nplanetmax,sol,nmax,1,time,
     .  itime,ntt,tobs,omc,tmodel,dtype)
      bs=(1.0d0-tmodel(1)+sol(8))*1.0d6
c      write(0,*) "tm: ",bs,tmodel(1)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nunit,nplanet,npt,np,seed,i,j,k,col,nmax,nfitm,ii
     ...
     .  adrs,rhostar,bsol(nfitm),bs,ave
...
c        read(5,*)
        ...
        temp(1)=Psec/Pi
        temp(2)=1.0d0/adrs
        temp(3)=(1.0+rdr)**2.0-bb
        temp(4)=1.0-cincl*cincl
C     Transit duration in hours
...
c            read(5,*)
 ...
 11   continue
      ...
      ii=8+(nplanet-1)*10
      adrs=1000.0*bsol(1)*G*(bsol(ii+2)*86400.0d0)**2/(3.0d0*Pi)
      adrs=adrs**(1.0d0/3.0d0) !a/R*
c      write(0,*) adrs,bsol(1),bsol(ii+2)
      cincl=bsol(ii+3)/adrs !cos(i)
c      write(0,*) cincl,bsol(ii+3)
      Psec=bsol(ii+2)*8.64d4

      temp(1)=Psec/Pi
      temp(2)=1.0d0/adrs
      temp(3)=(1.0d0+bsol(ii+4))**2.0-bsol(ii+3)*bsol(ii+3)
c      write(0,*) bsol(ii+4),bsol(ii+3)
      temp(4)=1.0d0-cincl*cincl
C     Transit duration in hours
      bs=temp(1)*asin(temp(2)*sqrt(temp(3)/temp(4)))/3600.0
      if((bs.gt.-9.9e30).and.(bs.lt.9.9e30))then
c         write(0,*) "bs: ",bs
         continue
      else
         bs=ave
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nunit,npt,np,seed,nplanet,i,j,k,col,nfitm,ii
...
c        read(5,*)
 ...
 11   continue
      ...
      ii=8+(nplanet-1)*10
      bs=1000.0*bsol(1)*G*(bsol(ii+2)*86400.0d0)**2/(3.0d0*Pi)
      bs=bs**(1.0d0/3.0d0)

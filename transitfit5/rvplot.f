     ...
     .  nplot,bins,order(10),jj
      parameter(nmax=2000000,nfit=108,nplot=1000)
...
c        do 24 i=1,nplanet
        rbb(3)=min(rbb(3),real(-sol(15+10*(j-1))))
        rbb(4)=max(rbb(4),real( sol(15+10*(j-1))))
c 24     continue
        rdiff=rbb(4)-rbb(3)
        rbb(3)=rbb(3)-rdiff*0.1
        rbb(4)=rbb(4)+rdiff*0.1
c        rbb(3)=-18.0
c        rbb(4)= 18.0
c        write(0,*) "rbb:",rbb(3),rbb(4)
        ...
        call pgslw(2)
        call pgsch(2.0)
c        if(j.eq.1) call pgvport(0.15,0.95,0.0,0.8)
c        if(j.eq.2) call pgvport(0.15,0.95,0.2,1.0)
        call pgvport(0.15,0.95,0.0,1.0)
        ...
        write(text,500) 'P=',sol(k2)
 500    format(A2,F8.3)
c        call pgtext((rbb(1)+rbb(2))/2.0,(rbb(3)+rbb(4))/2.0,text)
        call pgtext(rbb(1)+2.0*(rbb(2)-rbb(1))/3.1,
     .      rbb(3)+(rbb(4)-rbb(3))/15.0,text)

c        if(j.eq.1) call pglabel("" ,"RV residuals (m/s)","")
        if(jj.eq.nplanet) call pglabel("Phase","","")
...
c        read(5,*)  
 ...
 22   continue
      rbb(1)=-max(abs(rbb(1)),abs(rbb(3)))
      rbb(2)= max(abs(rbb(2)),abs(rbb(4)))
      rbb(3)=-max(abs(rbb(1)),abs(rbb(3)))
      rbb(4)= max(abs(rbb(2)),abs(rbb(4)))
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nfit,npt,i,j,nintg,dtype(npt),ii,nplanet,caltran
      parameter(nintg=11)
...
c        write(0,*) bs2,b
        ...
        ecw=sol(10*(ii-1)+8+6) !note that standard defn. are flipped.
        esw=sol(10*(ii-1)+8+5) !the input is correctly labeled.

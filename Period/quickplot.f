     ...
     .  nstarmax,niter,nstar,plot,nfmax,ma,itmax,npt2,j,nunit
      parameter(nmax=1650000,nstarmax=300,nfmax=2000)
      integer ndt(nmax)
     ...
     .  std(2),mean,stdev,time2(nmax),mag2(nmax),merr2(nmax),
     .  itime(nmax)
      character*80 cline,ans,title,plotdev,filename
      logical multi,loop
      ...
      call getarg(1,cline)
      read(cline,*) kID  !read in Kepler ID
      if(iargc().ge.2) then
        call getarg(2,cline)
...
C     These lines convert to parts-per-thousand
      do 11 i=1,npt
        ...
        time2(i)=time(i)!copy of original data
        mag2(i)=mag(i)
        merr2(i)=merr(i)
 11   continue
      npt2=npt
 ...
 5    continue
      ...
      if(kID.lt.10)then
        write(plotdev,507) kID,".ps/cps"
 507    format(I1,A7)      
      elseif(kID.lt.100)then
        write(plotdev,508) kID,".ps/cps"
 508    format(I2,A7)      
      elseif(kID.lt.1000)then
        write(plotdev,509) kID,".ps/cps"
 509    format(I3,A7)
      elseif(kID.lt.10000)then
        write(plotdev,510) kID,".ps/cps"
 510    format(I4,A7)
      elseif(kID.lt.100000)then
        write(plotdev,511) kID,".ps/cps"
 511    format(I5,A7)
      elseif(kID.lt.1000000)then
        write(plotdev,512) kID,".ps/cps"
 512    format(I6,A7)
      elseif(kID.lt.10000000)then
        write(plotdev,513) kID,".ps/cps"
 513    format(I7,A7)
      else
        write(plotdev,514) kID,".ps/cps"
 514    format(I8,A7)
      endif
      call pgopen(plotdev)
c      call pgopen('?')
...
C     sigma clipping
      do 10 i=1,1
        call sigup(npt,time,mag,merr,sig,rmavg)
 10   continue
...
c      call avgrm(npt,mag,rmavg) !make sure zero point is up-to-date
...
c      title=" "
c      call plotdata(npt,time,mag,merr,-1.0d0,-1.0d0,1,2,Keplertime,
c     .  title)
...
c      call windowfn(npt,nfit,time,mag,merr,bper,freq1,freq2,avg,a,nb,nc,
c     .   steps,1,3,nstar,pers)
      plot=0
...
c         call pdm(npt,time,res,nb,nc,per1,per2,steps,bper,btheta,1,4)
 ...
 23   continue
      ...
      title=" "
      call plotdata(npt2,time2,mag2,merr2,-1.0d0,-1.0d0,1,1,Keplertime,
     .  title)
      call plotfit(npt,nfit,time,mag,pers,avg,a,w,nstar)
      call sigup(npt,time,res,merr,sig,rmavg)
      title=" "
      call plotdata(npt,time,res,merr,-1.0d0,-1.0d0,1,2,Keplertime,
     .  title)
      call pgpanl(1,4)
c      call plotph(npt,time,mag,bper,1,4)
      call jmfourw(npt,time,res,merr,freq1,freq2,steps,bper,btheta,

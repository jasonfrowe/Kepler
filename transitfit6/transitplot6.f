      program transitfit6
      implicit none
      integer nrad,ntheta,CI1,CI2,P,nfit,nunit,nplanet,i,col,nmax,npt,
     .  nptv,npta
      parameter(nfit=125,nmax=600000)
      integer dtype(nmax),ia(nfit)
      real sign,contra,bright,rbb(3,4)
...
C     minpack
c      integer iwa(nfit),lwa
c      parameter(lwa=nmax*nfit+5*nmax*nfit)
c      double precision fvec(nmax),wa(lwa),sol2(nfit)
      character*80 inputsol,obsfile,rvfile,cline
      character*3 titles(26)
c      common /Fitting/ nrad,ntheta,npta,nplanet,aIT
c      common /Fitting2/ nrad,ntheta,nplanet,dtype,aT,aM,aE,aIT,sol,serr
      
      nrad=100   !number of rings to integrate star/planet surface 
      ntheta=1000 !maximum number of angular divisions 
...
c      call plotsetup(rbb,nfit,nplanet,sol,0)  

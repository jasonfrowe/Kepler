...
subroutine makeplot(nplot,nstep,pers,p,obsfile,epo,bper,bpow,depth,    &
...
real(double) :: epo,bper,phase,bpow,depth,pmean,std,qtran,medp
real(double), dimension(:) :: pers,p,time,flux
...
if(npt.ge.3)then
   allocate(ip(nstep))
   call rqsort(nstep,p,ip)
   medp=p(ip(nstep/2))
   call plot(nstep,pers,p-medp,obsfile,"Period (d)","P",0,0.0d0,0.0d0)
   call pgpage()

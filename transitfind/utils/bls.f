...
c
     ...
     &p,bper,bpow,depth,qtran,in1,in2,sn,psmooth)
c
c------------------------------------------------------------------------
c     >>>>>>>>>>>> This routine computes BLS spectrum <<<<<<<<<<<<<<
c
...
c
      implicit real*8 (a-h,o-z)
c
      dimension t(*),x(*),u(*),v(*),p(*),sn(*),psmooth(*)
      dimension y(8000),ibi(8000)
c
      minbin = 5
      nbmax  = 8000
      if(nb.gt.nbmax) write(*,*) ' NB > NBMAX !!'
...
C     Added by JR      
c      sn1=1.0!sn(jf)
c      ps1=0.0!psmooth(jf)
C     Added by JR
      sn1=sn(jf)
      ps1=psmooth(jf)

c      write(0,*) "Start bls",jf
...
c
      power = dsqrt(power)
c      write(0,*) "break",jf
      p(jf) = (power-ps1)/sn1!power
c      write(0,*) "break"
c
      if((power-ps1)/sn1.lt.bpow) go to 100
      bpow  =  (power-ps1)/sn1
      in1   =  jn1
      in2   =  jn2
      qtran =  rn3/rn

     ...
     .   koimat(nmax,nplanetmax),nfp(nmax)
...
c      do 45 i=1,nEB
c         if(kidEB(i).lt.10)then
c            write(6,505) "0000000",kidEB(i),"||TPS_EXCLUDE|||"
c 505        format(A7,I1,A16)
c         elseif(kidEB(i).lt.100)then
c            write(6,506) "000000",kidEB(i),"||TPS_EXCLUDE|||"
c 506        format(A6,I2,A16)
c         elseif(kidEB(i).lt.1000)then
c            write(6,507) "00000",kidEB(i),"||TPS_EXCLUDE|||"
c 507        format(A5,I3,A16)
c         elseif(kidEB(i).lt.10000)then
c            write(6,508) "0000",kidEB(i),"||TPS_EXCLUDE|||"
c 508        format(A4,I4,A16)
c         elseif(kidEB(i).lt.100000)then
c            write(6,509) "000",kidEB(i),"||TPS_EXCLUDE|||"
c 509        format(A3,I5,A16)
c         elseif(kidEB(i).lt.1000000)then
c            write(6,510) "00",kidEB(i),"||TPS_EXCLUDE|||"
c 510        format(A2,I6,A16)
c         elseif(kidEB(i).lt.10000000)then
c            write(6,511) "0",kidEB(i),"||TPS_EXCLUDE|||"
c 511        format(A1,I7,A16)
c         else
c            write(6,512) kidEB(i),"||TPS_EXCLUDE|||"
c 512        format(I8,A16)
c         endif
c 45   continue
...
C     count up number of planets in each KOI systems
 ...
 13   continue
      ...
      do 12 i=1,npt
         ...
         if(flag(i).eq.1) nfp(koinum)=nfp(koinum)+1
 12   continue
 ...
 31   continue
     ...
     .  title,ave,std,errs,1,datamin2,datamax2)
      ...
      do 32 i=1,npt
         koinum=int(koi(i)+0.1)
         if((np(koinum).ge.2).and.((flag(i).eq.1).or.(sn(i).le.10)
     .     .or.(per(i).lt.1.6).or.(rdr(i)+b(i).ge.0.98)))then
            j=j+1
            dp(j)=b(i)
            dp2(j)=rp(i)
         endif
...
C     count up number of planets in each KOI systems
 ...
 16   continue
      ...
      do 17 i=1,npt
         if((flag(i).lt.1).or.(flag(i).eq.2))then
            koinum=int(koi(i)+0.1)
            koimax=max(koinum,koimax) !largest KOI number
            np(koinum)=np(koinum)+1
         endif
...
C     count up number of planets in each KOI systems
 ...
 22   continue
      do 23 i=1,npt
         if((flag(i).lt.1).and.(sn(i).gt.10.0).and.(per(i).gt.1.6))then
            koinum=int(koi(i)+0.1)
            koimax=max(koinum,koimax) !largest KOI number
            np(koinum)=np(koinum)+1
         endif
...
c         if(flag(i).lt.1)then
         if((flag(i).lt.1).and.(sn(i).gt.10.0).and.(per(i).gt.1.6)
...
c     .       (koimat(i,j),j=1,nplanetmax)
 ...
 27   continue
     ...
     .     .and.(per(i).gt.1.6).and.(rdr(i)+b(i).lt.0.98))then
            j=j+1
            dp(j)=b(i)
            dp2(j)=rp(i)
         endif

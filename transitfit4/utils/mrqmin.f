...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      INTEGER na,n
      REAL*8 x,y,a(na),dyda(na),dtype
      integer nfit,npt,nmax,i,j
      parameter(nmax=2000000,nfit=18)
      integer ipars(nfit)
      real*8 exptime,itime(nmax),sol2(nfit),vary(nfit),
     .  y1,y2,s0,s1,s2
      common /Fitting/ npt,itime
c      data vary/2.0d-3,1.0d-4,1.0d-3,1.0d-4,1.0d-1,5.0d-5,1.0d-2,1.0d-2,
c     .  1.0,0.01,0.01,0.01,0.01,0.01,10.0,10.0,10.0,0.01/

      data vary/2.0d-4,1.0d-5,1.0d-3,1.0d-4,1.0d-2,5.0d-5,1.0d-2,1.0d-2,
     .  1.0,0.01,0.01,0.01,0.01,0.01,1.0,1.0,1.0,0.01/

      exptime=itime(n)
      call transitmodel(na,a,1,x,exptime,y,dtype)
...
c            endif
        ...
        sol2(i)=s1
        call transitmodel(na,sol2,1,x,exptime,y1,dtype)
c        call transitmodel(1,x,exptime,y1,nfit,sol2)
        sol2(i)=s2
        call transitmodel(na,sol2,1,x,exptime,y2,dtype)

      program mcmchisto
      implicit none
      integer nmax,npt,col,nbin,i,j,nfit,nunit,nbinmax,k,npt2,npt3
      parameter(nmax=5000000,nfit=22,nbinmax=500)
      integer nd(nmax)
     ...
     .  adrs(nmax),dd3(nmax)
      ...
      titles(1)='Epoch (HJD-2454900)'
      titles(2)='Period (days)'
      titles(3)='b'
      titles(4)="Rp/R*"
      titles(5)="z/R*"
      titles(6)='Phot zpt'
      titles(7)="e cos w"!"Eccentricity"
      titles(8)="e sin w"!"Argument of Pericenter (deg)"
      titles(9)="K (m/s)"
      titles(10)="velocity offset"
      titles(11)='Non-Linear Limbdarkening(1)'
      titles(12)='Non-Linear Limbdarkening(2)'
      titles(13)='Non-Linear Limbdarkening(3)'
      titles(14)='Non-Linear Limbdarkening(4)'
      titles(15)='Secondary Eclipse Depth (ppm)'
      titles(16)='Ellipsodial Amplitude (ppm)'
      titles(17)='Phase changes (ppm)'
      titles(18)='Dilution'
      titles(19)="e"
      titles(20)="w"
      titles(21)="a/R*"
      titles(22)="Transit Duration (h)"
...
C     Now we loop over all fitted parameters 
      do 10 i=1,18+4
         ...
         npt=nmax
         if(i.eq.19)then
            sol(19)=sqrt(sol(7)*sol(7)+sol(8)*sol(8))
            do 35 j=1,npt2
                eccn=sqrt(ecw(j)*ecw(j)+esw(j)*esw(j))
                dd(j)=eccn
 35         continue
         elseif(i.eq.20)then
            eccn=sqrt(sol(7)*sol(7)+sol(8)*sol(8))
            ...
            if((sol(7).gt.0.0e0).and.(sol(8).lt.0.0e0)) w=tPi+w
            if((sol(7).lt.0.0e0).and.(sol(8).gt.0.0e0)) w=Pi+w
            if((sol(7).lt.0.0e0).and.(sol(8).lt.0.0e0)) w=Pi+w
            sol(20)=w
            do 36 j=1,npt2
                eccn=sqrt(ecw(j)*ecw(j)+esw(j)*esw(j))
                if(eccn.ge.1.0) eccn=0.99
                ...
                if((ecw(j).gt.0.0e0).and.(esw(j).lt.0.0e0)) w=tPi+w
                if((ecw(j).lt.0.0e0).and.(esw(j).gt.0.0e0)) w=Pi+w
                if((ecw(j).lt.0.0e0).and.(esw(j).lt.0.0e0)) w=Pi+w
                dd(j)=w
 36         continue
         elseif(i.eq.21)then
            eccn=sqrt(sol(7)*sol(7)+sol(8)*sol(8))
            sol(21)=sol(5)*sol(2)/tpi*sqrt(1-sol(3))*(1+sol(8))/
     .              sqrt(1-eccn*eccn)
            do 37 j=1,npt2
                eccn=sqrt(ecw(j)*ecw(j)+esw(j)*esw(j))
                adrs(j)=zdr(j)*per(j)/tpi*sqrt(1-bb(j))*(1+esw(j))/
     .              sqrt(1-eccn*eccn)
                dd(j)=adrs(j)
c                if(dd(j).lt.5)write(0,*) j,dd(j),zdr(j),per(j)
c                write(0,*) npt2
c                read(5,*)
 37         continue
         elseif(i.eq.22)then
            incl=acos(sqrt(sol(3))/sol(21))
            tmp(1)=(1+sol(4))*(1+sol(4))
            tmp(2)=(sol(21)*cos(incl))*(sol(21)*cos(incl))
            tmp(3)=1-cos(incl)*cos(incl)
            tdur=sol(2)/pi*asin(sqrt((tmp(1)-tmp(2))/tmp(3))/sol(21))
            sol(22)=tdur*24.0
c            write(0,*) "Tdur: ",tdur*24.0
            do 38 j=1,npt2
                incl=acos(sqrt(bb(j))/adrs(j))
                tmp(1)=(1.0+rdr(j))*(1.0+rdr(j))
                tmp(2)=(adrs(j)*cos(incl))*(adrs(j)*cos(incl))
                tmp(3)=1-cos(incl)*cos(incl)
                tmp(4)=sqrt((tmp(1)-tmp(2))/tmp(3))
     .              /adrs(j)
                tdur=per(j)/pi*asin(tmp(4))
                if(((tmp(1)-tmp(2))/tmp(3).lt.0.0).or.
     .              (abs(tmp(4)).gt.1.0).or.
     .              (abs(sqrt(bb(j))/adrs(j)).gt.1.0)) tdur=0.0
                dd(j)=tdur*24.0
c                write(0,502) tdur,bb(j),incl,tmp(1),tmp(2),tmp(3)
c                read(5,*)
 38         continue
         else
...
C     These lines will sigclip at 4 sigma if required.  (probably not)
         ...
         do 12 j=1,npt
            if(abs(dd(j)-ave).lt.4.0*sqrt(var))then
               if((i.eq.22).and.(dd(j).le.0.0)) goto 40
               k=k+1
               dd3(k)=dd(j)
 40            continue
            endif
...
c         if(i.eq.6) sol(i)=ans(1,i)
         call errorest(npt,dd,nbin,bdatax,bdatay,bmax,mode(i),errs)
...
c         write(6,*) "npt:",npt
     ...
     .          (errs(j)*1.0e6,j=1,6)
         elseif(abs(sol(i)).ge.1.0d4)then
            write(6,501) sol(i),mode(i),ans(2,i),(errs(j),j=1,6)
         else
            write(6,500) sol(i),mode(i),ans(2,i),(errs(j),j=1,6)
         endif
...
c         read(10,*) dumi !not needed, but no harm. Just skips first line
 10   continue
      ...
      call exportfit(nfit,sol,serr,err,titles2)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine exportfit(nfit,sol,serr,err,titles)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer nfit,i
      ...
      do 10 i=1,nfit
        write(11,500) titles(i),sol(i),serr(i,1),serr(i,2),err(i)
 10   continue

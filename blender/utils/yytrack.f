...
C abs(xi(klo)-xbar)<1.
    ...
    2 if((khi-klo).gt.1)then
      ...
      if((khi-klo).le.0)then
                write(6,'(21h  interpolation error)')
                stop
      endif 
  522 continue
C now, (klo,khi) is sub-range of xi which contains xbar. 
      if(xi(klo).ne.0.0e0)then
        if(abs((xi(klo)-xbar)/xi(klo)).le.0.5e-5)L_one=1
      else
        if(abs(xi(klo)-xbar).le.0.5e-7)L_one=1
      endif
      return
      end
C track1 nline=24
  ...
  999 close(4)
   ...
   99 write(6,*)' error... file read in'
    ...
    9 format(i3,1x,5f12.8)
      enddo
      close(4)
      write(6,'(1x,8hOutput: ,a60)')filename
      return
      end 

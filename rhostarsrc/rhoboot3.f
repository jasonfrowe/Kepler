      program rhoboot3
C     searches for best-fit match and then calculates uncert from there
      implicit none
      integer iargc,nbin,nbinmax,nmax,nunit,np,i,nsol
      parameter(nmax=5000000,nbinmax=500,nsol=7)
      integer nd(nmax)
     ...
     .   sol(nsol),Msun,Rsun,G,logg
      character*80 rhofile,cline,title,name

C     Please note that these constants have been defined elsewhere
     ...
     .   nsol,sol)
      ...
      call pgopen('/null') !open PGPlot device
c      call pgopen('?')
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      implicit none
      integer np,nmax,nunit,i,nsol,j
      double precision mstar(nmax),age(nmax),z(nmax),rstar(nmax),
     .  rhostar(nmax),temp(nmax),lum(nmax),rchi,bchi,sol(nsol)

      bchi=1.0e30 !initiate to some large value.
     
      i=1
      j=0
      read(nunit,*,end=11) mstar(i),age(i),z(i),rstar(i),rhostar(i),
     .  temp(i),lum(i),rchi
      i=i+1
...
c      write(0,500) (sol(i),i=1,7),bchi
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     .  rave,errs(6),std,rmed,bsize,mode,momax
      double precision dp(npt),dp2(npt),ave,var,med,sol
      character*80 title
...
c      med=dp2(np(i))
c      rmed=real(med)
C     replace med,rmed with best-fit from MCMC chains
      med=sol-ave
      rmed=real(med)

      call avevar2(dp2,npt2,med,var)
      std=real(sqrt(var))
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE avevar2(data,n,ave,var)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     This version does not calculate average.  Instead uses input
C     and then calculates var centred on input value of ave.
      implicit none
      INTEGER n,m
      REAL*8 ave,var,data(n)
C Given array data(1:n), returns its mean as ave and its variance as var.

...
character(200) :: dumc
!atemp
integer,allocatable, dimension(:) :: nsteps,iPer
real(double) :: rtemp,vtemp,mutemp,atemp,Mtot
real(double), allocatable, dimension(:) :: ptemp,Pers
...
!atemp
allocate(ptemp(nbodies),nsteps(nbodies))
ptemp=0.0d0
nsteps=0.0d0
...
if (itprint.eq.0) then
	do j=2,nbodies
		np=7+7*(j-1)
  		Per=sol(np+2)
  		write(0,*) ptemp(j),nsteps(j)
  		percor(j)=ptemp(j)/dble(nsteps(j))-per

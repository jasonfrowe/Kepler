subroutine aei2xyz2(nbodies,sol,y,epoch)
use precision
implicit none
integer :: nbodies,i,j,np,ii,jj
integer, allocatable, dimension(:) :: iPer
real(double) :: adrs,Per,Gr,Pi,b,Pid2,esinw,ecosw,Psec,varpi,bige,      &
 Meanlong,Meananom,epoch,T0,eccanom,lame,bigm,K2,de,eps,bigen
...
do ii=1,nbodies
   ...
   b=abs(sol(np+3)) !impact parameter
   ...
   if(Per>0.0d0)then
      adrs=1000.0*sol(1)*Gr*(Per*86400.0d0)**2/(3.0d0*Pi)
      adrs=adrs**(1.0d0/3.0d0) !a/R*
      irad(i)=acos(b/adrs) !inclination radians  90 deg -> b=0
   else
...
K2 = 2.959122082855911d-4 !Gaussian gravitational constant squared (Mercury)
...
!   write(0,*) "MU: ",MU
   ...
   y(6*i)=y(6*i)*TU/LU
   write(0,500) m(i)/K2,(y(j),j=6*i-5,6*i),a(i)/LU
   500  format(28(1PE10.3,1X))
enddo

end subroutine aei2xyz2
